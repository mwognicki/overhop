...
- Exercises currently implemented protocol flow (`HELLO`, `REGISTER`, queue ops including `RMQUEUE`, `ENQUEUE`, `JOB`, `RMJOB`, subscriptions, `CREDIT`, `STATUS`, `PING`).
- Every self-debug `ENQUEUE` request is immediately followed by a `JOB` lookup using the returned `jid`.
- At the end of the flow, self-debug performs a final `ENQUEUE` and then `RMJOB` against that exact `jid`.
